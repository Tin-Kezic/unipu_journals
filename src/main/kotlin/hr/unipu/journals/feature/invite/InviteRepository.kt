...
interface InviteRepository: Repository<Invite, Int> {
    ...
    @Modifying
    @Query("DELETE FROM invite WHERE (email = :email OR :email IS NULL) AND (target = :target::invitation_target OR :target IS NULL) AND (target_id = :target_id OR :target_id IS NULL)")
    fun revoke(
